...
# 基本参数
# cmd: python eval.py [model_type] [pooling] [task_name] [dropout_rate]
# cmd: python eval.py BERT cls ATEC 0.3
...
# 加载数据集
root = '/Users/any/Downloads'
data_path = os.path.join(root, 'kg/senteval_cn') 
...
config_path = os.path.join(root, 'kg/bert/%s/bert_config.json' % model_name) 

if model_type == 'NEZHA':
    checkpoint_path = os.path.join(root, 'kg/bert/%s/model.ckpt-691689' % model_name)
elif model_type == 'NEZHA-large':
    checkpoint_path = os.path.join(root, 'kg/bert/%s/model.ckpt-346400' % model_name)
else:
    checkpoint_path = os.path.join(root, 'kg/bert/%s/bert_model.ckpt' % model_name)

dict_path = os.path.join(root, 'kg/bert/%s/vocab.txt' % model_name)
